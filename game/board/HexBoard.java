...
class HexBoard implements Board, Position {
    ...
    public Result makeMove(final Move move) {
        ...
        cells[move.getRow() - 1][move.getDiag() - 1] = move.getValue();

        int row = move.getRow() - 1;
        int diag = move.getDiag() - 1;
        if (checkWin(row, diag, 1, 0)
                || checkWin(row, diag, 0, 1)
                || checkWin(row, diag, -1, 1)) {
            return Result.WIN;
        }
        if (empty == 0) {
            return Result.DRAW;
    ...
    public boolean isValid(final Move move) {
        return 0 < move.getRow() && move.getRow() <= n
                && 0 < move.getDiag() && move.getDiag() <= m
                && cells[move.getRow() - 1][move.getDiag() - 1] == Cell.E
                && turn == getTurn();
    }
